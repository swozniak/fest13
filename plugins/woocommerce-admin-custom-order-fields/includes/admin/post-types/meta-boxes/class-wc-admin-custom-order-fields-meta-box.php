...
class WC_Admin_Custom_Order_Fields_Meta_Box {
	...
	public function render() {
		...
		$order_fields = $GLOBALS['wc_admin_custom_order_fields']->get_order_fields( $post->ID );
			...
			<?php foreach ( $order_fields as $field ) : ?>
				<li class="form-field">
					<label for="wc-admin-custom-order-fields-input-<?php echo esc_attr( $field->id ); ?>">
						<?php _e( $field->label, WC_Admin_Custom_Order_Fields::TEXT_DOMAIN ); ?> <?php if ( $field->is_required() ) echo '<span class="required">*</span>'; ?> <?php if ( isset( $field->description ) ) : ?><img class="help_tip" width="16" height="16"  data-tip='<?php echo $field->description ?>' src="<?php echo SV_WC_Plugin_Compatibility::WC()->plugin_url(); ?>/assets/images/help.png" /><?php endif; ?></label>
					<?php
					...
					switch( $field->type) :
						...
						case 'select':
						case 'multiselect':
							printf( '<select name="%s" id="%s" class="chosen_select" style="width: 70%%" %s/>', 'multiselect' === $field->type ? $name . '[]' : $name, $id, 'multiselect' === $field->type ? 'multiple="multiple"' : '' );
	...
	public function save( $order_id ) {
		...
		$order_fields = $GLOBALS['wc_admin_custom_order_fields']->get_order_fields();
