...
class WC_Admin_Custom_Order_Fields_Admin {
	...
	public function load_styles_scripts( $hook_suffix ) {
		global $wc_admin_custom_order_fields, $post_type, $wp_scripts;
		...
		if ( ( 'shop_order' == $post_type && ( 'post.php' == $hook_suffix || 'post-new.php' == $hook_suffix ) ) || $this->page_id === $hook_suffix ) {
			...
			if ( $this->page_id === $hook_suffix ) {
				...
				// admin JS
				wp_enqueue_script( 'wc-admin-custom-order-fields-admin', $wc_admin_custom_order_fields->get_plugin_url() . '/assets/js/admin/wc-admin-custom-order-fields' . $suffix . '.js', array( 'jquery', 'jquery-ui-sortable', 'woocommerce_admin' ), WC_Admin_Custom_Order_Fields::VERSION );
	...
	private function render_editor() {
		global $wc_admin_custom_order_fields;
				...
				<table class="widefat wc-admin-custom-order-fields-editor">
					<thead>
						<tr>
							...
							<th width="1%" class="wc-custom-order-field-attributes"><?php _e( 'Attributes', WC_Admin_Custom_Order_Fields::TEXT_DOMAIN ); ?></th>
	...
	private function save_custom_fields() {
		...
		if ( ! empty( $_POST['wc-custom-order-field-id'] ) ) {
			...
			for ( $index = 0; $index < count( $_POST['wc-custom-order-field-id'] ); $index++ ) {
				...
				// label
				$fields[ $field_id ]['label'] = sanitize_text_field( $_POST['wc-custom-order-field-label'][ $index ] );
				...
				if ( ! empty( $_POST['wc-custom-order-field-description'][ $index ] ) ) {
					$fields[ $field_id ]['description'] = sanitize_text_field( $_POST['wc-custom-order-field-description'][ $index ] );
				}
				...
				if ( ! empty( $_POST['wc-custom-order-field-default-values'][ $index ] ) ) {
					...
					switch ( $fields[ $field_id ]['type'] ) {
						...
						case 'text':
						case 'textarea':
							$fields[ $field_id ]['default'] = sanitize_text_field( $_POST['wc-custom-order-field-default-values'][ $index ] );
							break;
