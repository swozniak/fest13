jQuery( document ).ready( function( $ ) {
	...
	// enable chosen
	$editor.find( '.chosen_select' ).chosen();
	...
	$editor
		...
		.on( 'click', '.js-wc-admin-custom-order-fields-add-field', function () {
			...
			// enable chosen for the new field row selects/multiselects
			$editor.find( '.chosen_select' ).chosen();
		...
		.on( 'change', 'select.js-wc-custom-order-field-type', function() {
			...
			$attributes.trigger("liszt:updated");
		} )
	...
	$( 'form.wc-admin-custom-order-fields' ).submit( function( e ) {
