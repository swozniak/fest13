...
class WC_Admin_Custom_Order_Fields_Shop_Order_CPT {
	...
	public function render_column_titles( $columns ) {
		...
		// inject our columns
		foreach ( $GLOBALS['wc_admin_custom_order_fields']->get_order_fields() as $order_field ) {
	...
	public function render_column_content( $column ) {
		...
		foreach ( $GLOBALS['wc_admin_custom_order_fields']->get_order_fields( $post->ID ) as $order_field ) {
		...
		foreach ( $GLOBALS['wc_admin_custom_order_fields']->get_order_fields() as $order_field ) {
			...
			if ( $order_field->is_filterable() ) {
				...
				if ( $order_field->type == 'date' ) {
					...
					$javascript .= "jQuery( 'select#" . $order_field->get_meta_key() . "' ).chosen();";

				} elseif ( $order_field->has_options() ) {

					// filterable multi item field (select, multiselect, radio, checkbox), provide a dropdown
					?>
					<select name="<?php echo esc_attr( $order_field->get_meta_key() ); ?>" id="<?php echo esc_attr( $order_field->get_meta_key() ); ?>">
						<option value=""><?php _e( "Show all " . $order_field->label, 'wc_admin_custom_order_fields' ); ?></option>
						<?php
					...
					// search box dropdown
					?>
					<select id="<?php echo esc_attr( $order_field->get_meta_key() ); ?>" name="<?php echo esc_attr( $order_field->get_meta_key() ); ?>">
						<option value=""><?php _e( "Show all " . $order_field->label, 'wc_admin_custom_order_fields' ); ?></option>
						<?php
					...
					$javascript .= "
					jQuery( 'select#" . $order_field->get_meta_key() . "' ).ajaxChosen( {
						method:         'GET',
						...
						data: {
							...
							default:    '" . __( 'Show all ' . $order_field->label, 'wc_admin_custom_order_fields' ) . "'
						}
					}, function ( data ) {
	...
	private function render_months_dropdown( $display_name, $field_name ) {
		...
		<option<?php selected( $m, 0 ); ?> value="0"><?php _e( 'Show all ' . $display_name, 'wc_admin_custom_order_fields' ); ?></option>
		<?php
	...
	public function add_json_search_field() {
		...
		// the field to search
		$field_name = urldecode( stripslashes( strip_tags( $_GET['field_name'] ) ) );
